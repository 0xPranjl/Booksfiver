...
$id=$_GET['book'];
$q1="SELECT * FROM package WHERE sn='$id'";
$reso=mysqli_query($conn,$q1);
$ff=mysqli_fetch_assoc($reso);
$cmail=$ff['email'];
$pnaam=$ff['pnaam'];
$price=$ff['price'];
$pnaam=$ff['pnaam'];
...
$mk="ppay.php?sn=".$id;
$pid=$rt['pid'];
echo "<div class='col-sm' style='top: 50px;'>";
echo "<a href=ps.php?book=".$ff['sn'].">";
echo "<img src=".$f.' width=280px height="280px">';
echo "<h3>".$pnaam. "</h3>";
echo "<h4 style='color:green;'>(₹".$price.")</h4>";
echo "</a>";
echo "<a href=".$mk."><button class='bubbly-button'>Buy Now</button></a><br><br><br>";
echo "</div><br><br>";
$q2="SELECT * FROM package where email='$cmail' and pnaam='$pnaam'";
$s=mysqli_query($conn,$q2);
echo '<h1>Package Contains</h1><table class="table table-hover table-dark" style="margin-top:-650px;">
<thead>
  <tr>
    <th scope="col">#</th>
...
while($rt=mysqli_fetch_assoc($s)){
  ...
  $f=$u.$pic;
  $pid=$rt['pid'];
  $querys="SELECT * FROM product_info WHERE sn='$pid'";
